...
\begin{tikzpicture}[
  node distance=8pt,
  block/.style={rectangle, draw, rounded corners=2pt, minimum width=2.4cm,
                minimum height=0.85cm, align=center, font=\small},
  layer/.style={rectangle, draw=gray!50, dashed, rounded corners=4pt,
                inner sep=10pt, fill=blue!4},
...
% Parser layer
\node[block, fill=green!10, below=18pt of kb] (vla) {vla.py\\(Claude+keyword)};
\node[block, fill=green!10, right=of vla] (voice) {voice.py\\(fuzzy snap)};
\node[block, fill=green!10, right=of voice] (pipe) {pipeline.py\\(action$\to$plan)};

% Control layer
\node[block, fill=cyan!10, below=18pt of vla] (tasks) {tasks.py\\(waypoints)};
\node[block, fill=cyan!10, right=of tasks] (ctrl) {control.py\\(PD slew)};
\node[block, fill=cyan!10, right=of ctrl] (catch) {catcher.py\\(MPC+IK)};

% Physics layer
\node[block, fill=red!10, below=18pt of tasks] (phys) {physics.py};
\node[block, fill=red!10, right=of phys] (newt) {Newton XPBD};

% Render layer
\node[block, fill=purple!10, below=18pt of phys] (render) {scene/, scene\_legacy};
\node[block, fill=purple!10, right=of render] (effects) {effects.py};
\node[block, fill=purple!10, right=of effects] (sfx) {sfx.py};

% Telemetry
\node[block, fill=yellow!20, right=18pt of mic] (tel) {telemetry.py\\(CSV)};
...
\draw[arrow, dashed] (tel.south) -- ++(0,-0.3) -| (pipe.north);

\end{tikzpicture}
\caption{Demo Live 总体架构。输入层 (橙) $\to$ 解析层 (绿) $\to$
控制层 (青) $\to$ 物理层 (红) $\to$ 渲染层 (紫)，
telemetry (黄) 横切记录。}
\label{fig:arch}
\end{figure}
...
\begin{tikzpicture}[
  ...
  band/.style={rectangle, fill=#1, rounded corners=2pt, minimum height=18pt},
]
% Time axis
\draw[-Stealth, thick] (-0.3, 0) -- (12.5, 0) node[right] {$t$ (s)};
...
\node[above] at (0, 0.5) {\small ``stack a tower''};

% Preflight
\node[band=green!30, minimum width=4pt] (pre) at (0.1, 1.3) {};
\draw[->, thick, green!60!black] (0.1, 1.3) -- (0.5, 1.3);
\node[right] at (0.5, 1.3) {\small \textit{preflight 1 ms $\to$ arm 开始动}};

% Claude
\node[band=orange!40, minimum width=84pt] (claude) at (4.7, 0.7) {Claude};
\draw[<->, orange!70] (0.4, 0.3) -- (9, 0.3);
\node[above] at (4.7, 0.3) {\small 9.4 s};

% Refine
\node[event, fill=red!50] (refine) at (9, 1.3) {};
\node[above] at (9, 1.3) {\small Claude 返回，与 preflight 比较};

% Arm motion band
\node[band=blue!20, minimum width=10cm] (arm) at (5.5, 2.1) {};
\node at (5.5, 2.1) {\small Arm 动作：pick red $\to$ stack $\to \dots$};

\end{tikzpicture}
\caption{混合 VLA 解析管线时序。Preflight 在 1 ms 内派单
机械臂开始动，Claude 后台 9.4 s 后返回作为``智能复审''。}
\label{fig:hybrid}
\end{figure}
...
\subsection{演示截图}
\autoref{fig:industrial} 与 \autoref{fig:classroom} 展示两种
渲染模式的最终视觉。工业模式呈现 ``UR-class 双臂车间''
氛围；课堂模式则是``黑板上的手绘草稿''风格，适合不同观众。

\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{figures/industrial_view.png}
\caption{工业模式 (\texttt{--industrial})。左侧 Arm A 在
移动底盘上，右侧 Arm B 固定在支架上自主搬运 workpiece。}
...
\caption{默认课堂模式。手绘风格机械臂、彩色方块、控制提示
侧栏，模拟黑板教学场景。}
\label{fig:classroom}
\end{figure}
