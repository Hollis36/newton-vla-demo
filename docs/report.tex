...
\approx 6.67\ \text{cm}.
\]
\texttt{OFFSET\_SCHEDULE} $= (0, 4, 9)$\,cm 刻意包夹这一阈值：
$0$\,cm 为对齐基线，$4$\,cm（组合质心偏 $6$\,cm $< r$）仍稳，
$9$\,cm（组合质心偏 $13.5$\,cm $> r$）必倒。真实 XPBD 仿真在
课堂现场与该理论预测逐帧吻合，回归测试以
``$4$\,cm 稳 / $9$\,cm 倒'' 的包夹断言钉死这一行为。
...
真实 XPBD 判定倒塌。理论阈值 $d>6.67$\,cm 与仿真结果一致。}
...
\begin{itemize}[leftmargin=*, itemsep=0pt]
  ...
  \item \textbf{偏移塔稳定性物理实验} (\texttt{--experiment})：理论判据
    与真实 XPBD 仿真逐帧吻合；
