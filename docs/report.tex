...
\begin{itemize}[leftmargin=*, itemsep=0pt]
  ...
  \item \texttt{release\_block} 设回 \texttt{BodyFlags.DYNAMIC}，
    方块在重力下落到下方物体上完成堆叠；
  \item 翻转标志位后\textbf{必须}调用
    \texttt{solver.notify\_model\_changed(SolverNotifyFlags.BODY\_PROPERTIES)}，
    否则预计算的 kinematic 集不更新，翻转静默失效。
\end{itemize}
...
\textbf{\texttt{test\_experiment.py}} & 11 & \textbf{倒塌判据 4cm稳/9cm倒 包夹} \\
